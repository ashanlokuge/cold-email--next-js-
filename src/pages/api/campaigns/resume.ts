...
import { resumeCampaignInstance, getCampaignInstance, getCampaignStatus, getRunningCampaignsForUser } from '@/lib/multiCampaignManager';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!campaignId) {
      return res.status(400).json({ error: 'Campaign ID is required and no paused campaign found' });
    }
    ...
    await resumeCampaignInstance(campaignId);
